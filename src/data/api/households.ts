...
export interface Household {
  ...
  readonly overdueHorizonDays: number;
}

export interface Member {
  readonly userId: string;
  readonly displayName: string;
  readonly accent: 'blue' | 'pink';
...
export async function listMyHouseholds(): Promise<Household[]> {
  const { data, error } = await supabase
    .from('households')
    .select('id, name, time_zone, week_starts_on, overdue_horizon_days')
    .order('created_at');
  if (error) fail(error);
  ...
  return (data ?? []).map((row) => ({
    ...
    overdueHorizonDays: row.overdue_horizon_days,
  }));
}
...
export async function getHousehold(householdId: string): Promise<Household | null> {
  ...
  return {
    ...
    overdueHorizonDays: data.overdue_horizon_days,
  };
}
...
export async function listMembers(householdId: string): Promise<Member[]> {
  const { data, error } = await supabase
    ...
    .select('user_id, role, sort_order, profiles!inner(display_name, accent)')
    .eq('household_id', householdId)
    .order('sort_order');
  if (error) fail(error);

  return (data ?? []).map((row) => {
    const profile = row.profiles as unknown as { display_name: string; accent: string };
    return {
      userId: row.user_id,
      displayName: profile.display_name,
      accent: profile.accent === 'pink' ? 'pink' : 'blue',
      role: row.role,
...
export async function updateHousehold(
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      ...(patch.overdueHorizonDays !== undefined
        ? { overdue_horizon_days: patch.overdueHorizonDays }
        : {}),
    })
    .eq('id', householdId);
  if (error) fail(error);
