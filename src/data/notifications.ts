...
export function configureNotificationHandler(): void {
  Notifications.setNotificationHandler({
    // No banner and no sound while the app is open: the row a reminder refers
    // to is already on screen. It still goes to the notification list, so
    // nothing is lost by not interrupting.
    //
    // There used to be an exception for a "send a test reminder" button in
    // Settings; the button is gone, so the exception is too.
