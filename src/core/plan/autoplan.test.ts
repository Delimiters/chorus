...
describe('what a day fills itself with', () => {
  it('takes what is due and what is late', () => {
    ...
    expect(keys(autoPlannable(day, { userId: ME, on: TODAY, planned: NONE }))).toEqual([
      'v1:due',
      'v1:late',
    ]);
  });
  ...
  it('leaves out work that is not due yet', () => {
    ...
    expect(autoPlannable(day, { userId: ME, on: TODAY, planned: NONE })).toEqual([]);
  });
  ...
  it('leaves out what is already on the plan', () => {
    ...
    expect(keys(autoPlannable(day, { userId: ME, on: TODAY, planned }))).toEqual(['v1:b']);
  });

  it('takes one-off work too, which is a deliberate reversal', () => {
    /*
     * This used to exclude one-offs, on the argument that a one-off is a
     * decision and the proposal is where decisions belong. Jake asked for the
     * opposite. The rule no longer looks at the chore's schedule at all, so
     * there is nothing left to distinguish them by — which is the point.
     */
    const day = [item('recurring'), item('oneOff')];
    expect(keys(autoPlannable(day, { userId: ME, on: TODAY, planned: NONE }))).toEqual([
      'v1:recurring',
      'v1:oneOff',
    ]);
  ...
  it('leaves out the other person&apos;s assigned work', () => {
    ...
    expect(keys(autoPlannable(day, { userId: ME, on: TODAY, planned: NONE }))).toEqual(['v1:mine']);
  });
  ...
  it('answers for a person who is not the one asking', () => {
    ...
    expect(keys(autoPlannable(day, { userId: THEM, on: TODAY, planned: NONE }))).toEqual([
      'v1:shared',
      'v1:theirs',
    ]);
  });
